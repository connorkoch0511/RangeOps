...
public class TelemetryReadingTests
{
    ...
    {
        var line = "{\"alt_ft\":12345.6,\"airspeed_kt\":320.4,\"vs_fpm\":1800.0,\"fault\":false}";
        ...
        Assert.False(reading.Value.Fault);
    }

    [Fact]
    public void Parse_FaultTrue_SetsFaultFlag()
    {
        var reading = TelemetryReading.Parse(
            "{\"alt_ft\":674.8,\"airspeed_kt\":184.4,\"vs_fpm\":2036.9,\"fault\":true}");

        Assert.NotNull(reading);
        Assert.True(reading!.Value.Fault);
    }
    ...
    [InlineData("{\"alt_ft\":\"oops\",\"airspeed_kt\":1,\"vs_fpm\":1,\"fault\":false}")]
